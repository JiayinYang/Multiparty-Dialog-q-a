...
def prune_data(in_file):
    ...
    for sample in samples:
        question1 = sample['question1'].lower()
        question2 = sample['question2'].lower()
        document = []
    ...
    temp_1 = list(sorted_doc_frequency.keys())[:int(len(doc_frequency) * 0.20)]
...
def load_jsondata(in_file, redundent_1, redundent_2, stopwords_file):
    ...
    for sample in samples:
        if "question1" in sample:
            question = sample['question1'].lower()
        else:
            continue
        if "answer1" in sample:
            answer = sample['answer1'].lower()
        else:
            continue
        document = []
        num += 1
        ...
        if q_words[0] == "who" and len(answer.split()) == 1 and answer[0] == "@":
    ...
    for sample in samples:
        if "question2" in sample:
            question = sample['question2'].lower()
        else:
            continue
        if "answer2" in sample:
            answer = str(sample['answer2']).lower()
        else:
            continue
        document = []
        num += 1
    ...
    print('max_utterance_length %d' % (max_d_len))
    print('max_query_length %d' % (max_q_len))
    print('max_dialog_length %d' % (max_s_len))
...
def get_dim(in_file):
    line = open(in_file, encoding='utf-8').readline()
